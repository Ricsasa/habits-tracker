...
// No endpoint pages yet, so an unbounded select is a request-sized hole: one
// account with a large history makes every list response arbitrarily large.
...
export async function listTags(
  ...
  return unwrap(await query.order('name').limit(MAX_ROWS));
}
...
export async function listActivities(
  ...
  return unwrap(await query.order('start_time', { ascending: false }).limit(MAX_ROWS));
}
...
export async function filterActivities(
  ...
  if (filters.titleSearch) {
    query = query.ilike('title', `%${escapeLikePattern(filters.titleSearch)}%`);
  }
  return unwrap(await query.order('activity_date', { ascending: false }).limit(MAX_ROWS));
}
